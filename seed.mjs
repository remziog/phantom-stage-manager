...
type Seed = { email: string; password: string; full_name: string; company: string; industry: "rental" | "warehouse" | "logistics" | "mixed" };
...
function slugify(name: string) {
  return name.toLowerCase().replace(/[^a-z0-9\s-]/g, "").trim().replace(/\s+/g, "-").slice(0, 40) + "-" + Math.random().toString(36).slice(2, 6);
}

async function ensureUser(s: Seed) {
  ...
  // Sample data only for rental owner
...
(async () => {
  ...
  for (const s of seeds) {
    try {
      const r = await ensureUser(s);
      out.push({ email: s.email, password: s.password, ...r });
    } catch (e: any) {
      console.error(`FAIL ${s.email}:`, e.message);
    }
  }
  console.log("\n=== TEST USERS ===");
  console.table(out.map(o => ({ email: o.email, password: o.password, company: o.companyId })));
})();
