  ...
  return (
    <Card className="phantom-shadow border-border/50">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-medium text-foreground">Aylık Gelir (Son 12 Ay)</CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
              ...
              <Tooltip
                ...
                formatter={(value: number) => [new Intl.NumberFormat("tr-TR", { style: "currency", currency: "TRY", maximumFractionDigits: 0 }).format(value), "Gelir"]}
              />
...
export function EquipmentUtilizationPieChart({ equipment }: { equipment: Equipment[] }) {
  const data = useMemo(() => {
    ...
    equipment.forEach((e) => { counts[e.current_location] = (counts[e.current_location] || 0) + e.quantity_total; });
    return Object.entries(counts).map(([name, value]) => ({ name: locationLabels[name] || name, value })).filter((d) => d.value > 0);
  }, [equipment]);
  ...
  return (
    <Card className="phantom-shadow border-border/50">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-medium text-foreground">Ekipman Kullanımı</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="h-[260px]">
          {total === 0 ? (
            <div className="flex items-center justify-center h-full">
              <p className="text-sm text-muted-foreground">Ekipman verisi yok</p>
            </div>
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip
                  contentStyle={{ backgroundColor: "hsl(222, 47%, 9%)", border: "1px solid hsl(222, 30%, 18%)", borderRadius: "8px", color: "hsl(210, 40%, 98%)", fontSize: 12 }}
                  formatter={(value: number) => [`${value} ürün (%${Math.round((value / total) * 100)})`, ""]}
                />
              </PieChart>
            </ResponsiveContainer>
          )}
