    ...
    <div className="space-y-3">
      ...
      <div className="rounded-lg border border-border overflow-hidden">
        ...
        <div className="grid grid-cols-7 bg-secondary/50">
          {DAYS.map((d) => (
            <div key={d} className="px-2 py-1.5 text-center text-xs font-medium text-muted-foreground border-b border-border">
              {d}
            </div>
          ))}
        </div>
              ...
              return (
                <div
                  ...
                  className={`min-h-[80px] border-b border-r border-border p-1 transition-colors ${
                    !inMonth ? "bg-secondary/20" : "bg-card"
                  } ${di === 6 ? "border-r-0" : ""} ${wi === cells.length - 1 ? "border-b-0" : ""}`}
                >
                  ...
                  <div className="space-y-0.5">
                    {dayEvents.slice(0, 3).map((ev) => (
                      <button
                        ...
                        className="w-full flex items-center gap-1 rounded px-1 py-0.5 text-[10px] leading-tight text-foreground truncate hover:bg-secondary/80 transition-colors text-left"
                        title={`${ev.name} — ${ev.customer_name}`}
                      >
